...
const SocialTooltip = React.forwardRef<HTMLUListElement, SocialTooltipProps>(
  ({ className, items, ...props }, ref) => {
    ...
    const baseIconStyles =
      "relative flex items-center justify-center w-12 h-12 rounded-full bg-green-500/60 overflow-hidden transition-all duration-300 ease-in-out group-hover:shadow-lg border border-gray-700";
    const baseSvgStyles =
      "relative z-10 w-6 h-6 transition-colors duration-300 ease-in-out group-hover:text-white";
    ...
    return (
      <ul
        ref={ref}
        className={cn("flex items-center text-green-400 justify-center gap-4", className)}
        {...props}
      >
